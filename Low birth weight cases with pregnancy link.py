...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 12 14:13:12 2022
...
''' Select just those records where birth-weight is not null'''


df=df[df['birth_weight_kg']>0]
...
''' Outcome variable - if the child is reported as severe underweight <2.5kg then assign 1 else 0 '''
...
''' number of pregnancies linked with children  in a camp'''
df['number_of_child_linked_pregnancies']=df.groupby(['camp_id'])['child_linked_to_mother'].transform('sum')  
...
''' proportion of child linked pregnancies in a camp'''
df['proportion_of_child_linked_pregnancies']=df['number_of_child_linked_pregnancies']*100/df['number_of_children_registered']    

#df=df[df['proportion_of_child_linked_pregnancies']>=25]
df['suspected_sam_cases']=np.where(((df['sam_at_birth']=="yes")) | ((df['sam_at_1st checkup']=="yes")) | ((df['sam_at_2nd checkup']=="yes")) | ((df['sam_at_3rd checkup']=="yes")) | ((df['sam_at_4th checkup']=="yes")) | ((df['sam_at_5th checkup']=="yes")) | ((df['sam_at_6th checkup']=="yes")) | ((df['sam_at_7th checkup']=="yes")) | ((df['sam_at_8th checkup']=="yes")),1,0)
...
z1=df[df['suspected_sam_cases']==1]


''' Select just those records where pregnancy_id is not null'''
df=df[df['pregnancy_id'].notnull()]
df=df[df['camp_id'].isin(z1['camp_id'].unique().tolist())]
...
ancp = os.path.join("KhushiBaby_Cleaned_Anonymized_Dataset2.csv")
d12=pd.read_csv(ancp)
ancp = pd.read_csv(ancp)[['pregnancy_id','delivery_type','term_of_baby','delivery_complications']]
ancp=ancp.sort_values(['pregnancy_id','delivery_type','term_of_baby','delivery_complications'], ascending=False)
ancp=(ancp.drop_duplicates(subset='pregnancy_id', keep='first'))
ancp['term_of_baby']=ancp['term_of_baby'].replace("2021-11-01",'Full-term')
ancp['term_of_baby']=ancp['term_of_baby'].replace("2021-09-20",'Full-term')

df= pd.merge(df,ancp, on="pregnancy_id", how='left')
df['delivery_complications']=np.where((df['delivery_complications'].notnull()) ,1,0)
#model = smf.logit("z ~ C(gender, Treatment('M'))+ C(occupation_status, Treatment('Labour'))+ C(marital_status, Treatment('Married'))+C(TB, Treatment('No'))+age_40_60+C(lung_disease, Treatment('No'))+C(hypertension, Treatment('No'))+C(cancer, Treatment('No'))+C(bpl, Treatment('No'))+C(education_status, Treatment('No formal education'))+C(liver_disease, Treatment('No'))+C(religion, Treatment('Hindu'))", data=df)
df['term_of_baby']=df['term_of_baby'].replace('Full term','Full-term')
df['term_of_baby']=df['term_of_baby'].replace('Pre term','Pre-term')
...
#df= pd.merge(df,cmpz[['camp_id', 'performance_type']], on="camp_id", how='left')  
...
#C(term_of_baby, Treatment('Full-term'))
#df.household_visit_by_kb_monitors=df.household_visit_by_kb_monitors.replace("Yes",1)
#df.household_visit_by_kb_monitors=df.household_visit_by_kb_monitors.fillna(0)
#df.delivery_type=df.delivery_type.fillna("Normal")
...
child_sam['proportion_of_anemic_cases_reported_in_village']=child_sam['number_of_pregnant_women_with_hb_<=_8_from_kb_rch_dataset']/child_sam['number_of_pregnant_women_registered_per_kb_rch_dataset']
...
child_sam['proportion_of_confirmed_sam_cases_reported_in_village']=child_sam['number_of_sam_children_identified_by_kb_monitor']/child_sam['number_of_suspected_sam_household_visits_by_kb_monitor']
...
'''
df['performance_type']=df['performance_type'].replace(0,"Top Performer")
df['performance_type']=df['performance_type'].replace(1,"Good Performer")
df['performance_type']=df['performance_type'].replace(2,"Moderate Performer")
df['performance_type']=df['performance_type'].replace(3,"Poor Performer")
'''
df['total_anc_completed']=np.where((df['anc_date_4'].notnull()),4,np.nan)
df['total_anc_completed']=np.where((df['anc_date_3'].notnull()) & (df['anc_date_4'].isnull()) ,3,df['total_anc_completed'])
df['total_anc_completed']=np.where((df['anc_date_2'].notnull()) & (df['anc_date_3'].isnull()) & (df['anc_date_4'].isnull()) ,2,df['total_anc_completed'])
df['total_anc_completed']=np.where((df['anc_date_1'].notnull()) & (df['anc_date_2'].isnull()) & (df['anc_date_3'].isnull()) & (df['anc_date_4'].isnull()) ,1,df['total_anc_completed'])
...
df['term_of_baby']=df['term_of_baby'].replace('Full-term',1)
df['term_of_baby']=df['term_of_baby'].replace('Pre-term',0)

df=df.rename(index=str, columns={'high_risk_4':"high_risk_during_4th_anc","term_of_baby":"full_term_delivery"})

model = smf.logit("low_birth_weight ~high_risk_during_4th_anc+weight_2+weight_3+distance_from_phc_to_nearest_camp_in_kms+four_anc_completed+days_to_edd_from_last_checkup+high_risk_during_4th_anc+full_term_delivery", data=df)
...
#model_odds.index=np.where((model_odds.index=="high_risk_4"),"high_risk_during_4th_anc",model_odds.index)
#model_odds.index=np.where((model_odds.index=="term_of_baby"),"full_term_delivery",model_odds.index)

writer = ExcelWriter('Low_Birth_Weight_Regression_Analysis.xlsx')
...
plt.title('Logistic Regression (Low birth weight case with pregnancy link)')
plt.savefig('Low_Birth_Weight_ANC_Regression_Analysis.png', transparent=False)
...
X_cols = ['weight_2','weight_3','distance_from_phc_to_nearest_camp_in_kms','four_anc_completed','days_to_edd_from_last_checkup','high_risk_during_4th_anc','full_term_delivery']
Xx = df[X_cols]
...
'''Linear assumptions '''
df_titanic_lt=df[(df['weight_2']>0) & (df['weight_3']>0) & (df['distance_from_phc_to_nearest_camp_in_kms']>0) & (df['days_to_edd_from_last_checkup']>0)][['low_birth_weight','weight_2','weight_3','distance_from_phc_to_nearest_camp_in_kms','four_anc_completed','days_to_edd_from_last_checkup','high_risk_during_4th_anc','full_term_delivery']]
# Define continuous variables
continuous_var = ['weight_2','weight_3','distance_from_phc_to_nearest_camp_in_kms','days_to_edd_from_last_checkup']
...
y_lt = df_titanic_lt[['low_birth_weight']]
# Add constant
...
cols_to_keep = continuous_var + df_titanic_lt.columns.tolist()[-len(continuous_var):]
# Redefine independent variables to include interaction terms
df_titanic_lt['weight_3']=df_titanic_lt['weight_3']**5
df_titanic_lt['distance_from_phc_to_nearest_camp_in_kms']=df_titanic_lt['distance_from_phc_to_nearest_camp_in_kms']**5
df_titanic_lt['days_to_edd_from_last_checkup']=df_titanic_lt['days_to_edd_from_last_checkup']**5
import numpy as np
from sklearn.experimental import enable_iterative_imputer
from sklearn.impute import IterativeImputer
from sklearn.linear_model import LinearRegression
...
print(logit_results.summary())
'''
df_titanic_lt=df[['low_birth_weight','weight_2','weight_3','distance_from_phc_to_nearest_camp_in_kms','four_anc_completed','days_to_edd_from_last_checkup','high_risk_during_4th_anc','full_term_delivery']]
...
cols_to_keep =['weight_2','weight_3','distance_from_phc_to_nearest_camp_in_kms','days_to_edd_from_last_checkup','four_anc_completed','high_risk_during_4th_anc','full_term_delivery']
...
plt.scatter(x=X_lt['days_to_edd_from_last_checkup'].values, y=log_odds);
plt.xlabel("Days to edd from last checkup")
plt.ylabel("Log-odds")
plt.show()


# Visualize predictor continuous variable vs logit values 
plt.scatter(x=X_lt['distance_from_phc_to_nearest_camp_in_kms'].values, y=log_odds);
plt.xlabel("Distance to phc from nearest camp (kms)")
plt.ylabel("Log-odds")
plt.show()


# Visualize predictor continuous variable vs logit values 
plt.scatter(x=X_lt['weight_2'].values, y=log_odds);
plt.xlabel("Weight during ANC 2")
plt.ylabel("Log-odds")
plt.show()

# Visualize predictor continuous variable vs logit values 
plt.scatter(x=X_lt['weight_3'].values, y=log_odds);
plt.xlabel("Weight during ANC 3")
plt.ylabel("Log-odds")
plt.show()
...
anc=df[['low_birth_weight','weight_2','weight_3','distance_from_phc_to_nearest_camp_in_kms','four_anc_completed','days_to_edd_from_last_checkup','high_risk_during_4th_anc','full_term_delivery']]
...
import matplotlib.pyplot as plt
X_lt = sm.add_constant(anc[['weight_2','weight_3','distance_from_phc_to_nearest_camp_in_kms','four_anc_completed','days_to_edd_from_last_checkup','high_risk_during_4th_anc','full_term_delivery']], prepend=False)
y_lt=anc[['low_birth_weight']]
# Re-run logistic regression on original set of X and y variables
logit_results = GLM(y_lt, X_lt, family=families.Binomial()).fit()
predicted = logit_results.predict(X_lt)
...
writercsv = os.path.join("low_birth_weight_assumptions_check.csv")
...
X_train,X_test, y_train, y_test = train_test_split(logs_train[['weight_2','weight_3','distance_from_phc_to_nearest_camp_in_kms','days_to_edd_from_last_checkup','four_anc_completed','high_risk_during_4th_anc','full_term_delivery']], logs_train['low_birth_weight'], test_size=0.1,  random_state=random.seed(1234))


np.random.seed(1234)
oversample = SMOTE()
''' over sampling of minority class'''
over_X, over_y = oversample.fit_resample(logs_train[['weight_2','weight_3','distance_from_phc_to_nearest_camp_in_kms','days_to_edd_from_last_checkup','four_anc_completed','high_risk_during_4th_anc','full_term_delivery']], logs_train['low_birth_weight'])
...
X_train, X_test, y_train, y_test = train_test_split(logs_train[['weight_2','weight_3','distance_from_phc_to_nearest_camp_in_kms','days_to_edd_from_last_checkup','four_anc_completed','high_risk_during_4th_anc','full_term_delivery']], logs_train['low_birth_weight'], test_size=0.2, stratify=logs_train['low_birth_weight'], random_state=random.seed(1234))


SMOTE_SRF.fit(over_X_train, over_y_train)
#SMOTE SRF prediction result
y_pred = SMOTE_SRF.predict(logs_train[['weight_2','weight_3','distance_from_phc_to_nearest_camp_in_kms','days_to_edd_from_last_checkup','four_anc_completed','high_risk_during_4th_anc','full_term_delivery']])
#Create confusion matrix
fig = plot_confusion_matrix(SMOTE_SRF, logs_train[['weight_2','weight_3','distance_from_phc_to_nearest_camp_in_kms','days_to_edd_from_last_checkup','four_anc_completed','high_risk_during_4th_anc','full_term_delivery']], logs_train['low_birth_weight'], display_labels=['Low Birth Weight', 'Normal'], cmap='Greens')
plt.title('SMOTE + Standard Random Forest Confusion Matrix')
plt.show()

from sklearn.metrics import confusion_matrix
cf_matrix = confusion_matrix(logs_train['low_birth_weight'], y_pred)
print(cf_matrix)
...
#Create confusion matrix
fig = plot_confusion_matrix(SMOTE_SRF, X_test, y_test, display_labels=['Low Birth Weight', 'Normal'], cmap='Reds')
...
for column in ['weight_2','weight_3','distance_from_phc_to_nearest_camp_in_kms','four_anc_completed','days_to_edd_from_last_checkup','high_risk_during_4th_anc','full_term_delivery']:
...
oversample = SMOTE()
y=part_9[['low_birth_weight']]

X=part_9[['weight_2','weight_3','distance_from_phc_to_nearest_camp_in_kms','four_anc_completed','days_to_edd_from_last_checkup','high_risk_during_4th_anc','full_term_delivery']]
...
#Create confusion matrix
fig = plot_confusion_matrix(SMOTE_SRF, X ,y, display_labels=['Normal', 'Low birth weight'], cmap='Greens')
plt.title('SMOTE + Standard Random Forest Confusion Matrix')
plt.show()
...
y_test=rest_part_25[['low_birth_weight']]

X_test=rest_part_25[['weight_2','weight_3','distance_from_phc_to_nearest_camp_in_kms','four_anc_completed','days_to_edd_from_last_checkup','high_risk_during_4th_anc','full_term_delivery']]
...
y_pred = SMOTE_SRF.predict(X_test)
#Create confusion matrix
fig = plot_confusion_matrix(SMOTE_SRF, X_test, y_test, display_labels=['Normal', 'Low birth weight'], cmap='Reds')
plt.title('SMOTE + Standard Random Forest Confusion Matrix')
plt.show()
...
ax.set_title('Low Birth Weight model - Random Forest');
ax.set_xlabel('\nThe Truth')
ax.set_ylabel('Test Score ');

## Ticket labels - List must be in alphabetical order
ax.xaxis.set_ticklabels(['Low Birth Weight (Positive)','Normal (Negative)'])
...
ax.set_title('Low Birth Weight Model - Deep Neural Network');
ax.set_xlabel('\nThe Truth')
ax.set_ylabel('Test Score ');
...
# title
plt.title('ROC Curve - Low Birth Weight model')
...
rf_20=sensitivity_check(training_features,training_target,0.2,'Low Birth Weight Model - Random Forest (20% Over Sampling)',['Low Birth Weight (Positive)','Normal (Negative)'])
rf_30=sensitivity_check(training_features,training_target,0.3,'Low Birth Weight Model - Random Forest (30% Over Sampling)',['Low Birth Weight (Positive)','Normal (Negative)'])
rf_50=sensitivity_check(training_features,training_target,0.5,'Low Birth Weight Model - Random Forest (50% Over Sampling)',['Low Birth Weight (Positive)','Normal (Negative)'])
rf_70=sensitivity_check(training_features,training_target,0.7,'Low Birth Weight Model - Random Forest (70% Over Sampling)',['Low Birth Weight (Positive)','Normal (Negative)'])
rf_90=sensitivity_check(training_features,training_target,0.9,'Low Birth Weight Model - Random Forest (90% Over Sampling)',['Low Birth Weight (Positive)','Normal (Negative)'])
import shap
import pandas as pd
