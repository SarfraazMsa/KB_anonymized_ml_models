...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 12 14:13:11 2022
...
''' Outcome variable - if the child is reported as severe underweight (-3sd WHO standard) before 2 months from date of birth then assign 1 else 0 '''
df['severe_underweight']=np.where(((df['sam_at_birth']=="yes") & ((df['first_diffc'] <=60) | (df['second_diffc'] <=60) | (df['third_diffc'] <=60) | (df['fourth_diffc'] <=60) | (df['fifth_diffc'] <=60))) | ((df['sam_at_1st checkup']=="yes") & ((df['first_diffc'] <=60) | (df['second_diffc'] <=60) | (df['third_diffc'] <=60) | (df['fourth_diffc'] <=60) | (df['fifth_diffc'] <=60))) | ((df['sam_at_2nd checkup']=="yes") & ((df['first_diffc'] <=60) | (df['second_diffc'] <=60) | (df['third_diffc'] <=60) | (df['fourth_diffc'] <=60) | (df['fifth_diffc'] <=60))) | ((df['sam_at_3rd checkup']=="yes") & ((df['first_diffc'] <=60) | (df['second_diffc'] <=60) | (df['third_diffc'] <=60) | (df['fourth_diffc'] <=60) | (df['fifth_diffc'] <=60))) | ((df['sam_at_4th checkup']=="yes") & ((df['first_diffc'] <=60) | (df['second_diffc'] <=60) | (df['third_diffc'] <=60) | (df['fourth_diffc'] <=60) | (df['fifth_diffc'] <=60))) | ((df['sam_at_5th checkup']=="yes") & ((df['first_diffc'] <=60) | (df['second_diffc'] <=60) | (df['third_diffc'] <=60) | (df['fourth_diffc'] <=60) | (df['fifth_diffc'] <=60))) ,1,0)
...
''' discarding those mother where atleast 25% of children are not linked with pregnancy_id so that the model is not biased'''
...
#model = smf.logit("severe_underweight ~days_to_edd_from_last_checkup+high_risk_during_4th_anc+proportion_of_anemic_cases_reported_in_village+full_term_delivery+mother_age_greater_than_20_less_than_30+pregnancy_greater_than_3+registered_within_12_weeks+tt_booster_given", data=df)
model = smf.logit("severe_underweight ~hb_1+bp_sys_1+registered_within_12_weeks+weight_1+distance_from_phc_to_nearest_camp_in_kms+tt_2_given+high_risk_during_4th_anc+mother_age_greater_than_25_less_than_30", data=df)
...
writer = ExcelWriter('Child_Severe_Underweight_Before_2_months_ANC_Regression_Analysis.xlsx')
...
plt.title('Logistic Regression (Severe underweight <= 2 months case with pregnancy link)')
plt.savefig('Child_Severe_Underweight_Before_2_months_ANC_Regression_Analysis.png', transparent=False)
...
X_cols = ['hb_1','bp_sys_1','registered_within_12_weeks','weight_1','distance_from_phc_to_nearest_camp_in_kms','tt_2_given','high_risk_during_4th_anc','mother_age_greater_than_25_less_than_30']
Xx = df[X_cols]
...
'''Linear assumptions '''
df_titanic_lt=df[(df['hb_1']>0) & (df['weight_1']>0) & (df['distance_from_phc_to_nearest_camp_in_kms']>0) & (df['bp_sys_1']>0)][['severe_underweight','hb_1','bp_sys_1','weight_1','distance_from_phc_to_nearest_camp_in_kms','registered_within_12_weeks','tt_2_given','high_risk_during_4th_anc','mother_age_greater_than_25_less_than_30']]
# Define continuous variables
continuous_var = ['hb_1','bp_sys_1','weight_1','distance_from_phc_to_nearest_camp_in_kms']
...
# Keep columns related to continuous variables
cols_to_keep = continuous_var + df_titanic_lt.columns.tolist()[-len(continuous_var):]
df_titanic_lt['distance_from_phc_to_nearest_camp_in_kms']=((df_titanic_lt['distance_from_phc_to_nearest_camp_in_kms'])**5)/df_titanic_lt['distance_from_phc_to_nearest_camp_in_kms'].max()
# Redefine independent variables to include interaction terms
#df_titanic_lt['anm_average_data_quality_score']=df_titanic_lt['anm_average_data_quality_score']**5
import numpy as np
from sklearn.experimental import enable_iterative_imputer
...
print(logit_results.summary())
'''
df_titanic_lt=df[['severe_underweight','hb_1','bp_sys_1','weight_1','registered_within_12_weeks','tt_2_given','high_risk_during_4th_anc','mother_age_greater_than_25_less_than_30']]
...
cols_to_keep =['hb_1','bp_sys_1','registered_within_12_weeks','weight_1','tt_2_given','high_risk_during_4th_anc','mother_age_greater_than_25_less_than_30']
...
plt.scatter(x=X_lt['hb_1'].values, y=log_odds);
plt.xlabel("Haemoglobin recorded during ANC 1")
plt.ylabel("Log-odds")
plt.show()
...
plt.xlabel("Systolic blood pressure recorded during ANC 1")
plt.ylabel("Log-odds")
plt.show()
...
anc=df[['severe_underweight','hb_1','bp_sys_1','weight_1','registered_within_12_weeks','tt_2_given','high_risk_during_4th_anc','mother_age_greater_than_25_less_than_30']]
...
import matplotlib.pyplot as plt
X_lt = sm.add_constant(anc[['hb_1','bp_sys_1','weight_1','registered_within_12_weeks','tt_2_given','high_risk_during_4th_anc','mother_age_greater_than_25_less_than_30']], prepend=False)
...
X_train,X_test, y_train, y_test = train_test_split(logs_train[['hb_1','bp_sys_1','registered_within_12_weeks','weight_1','tt_2_given','high_risk_during_4th_anc','mother_age_greater_than_25_less_than_30']], logs_train['severe_underweight'], test_size=0.1,  random_state=random.seed(1234))


np.random.seed(1234)
oversample = SMOTE()
''' over sampling of minority class'''
over_X, over_y = oversample.fit_resample(logs_train[['hb_1','bp_sys_1','registered_within_12_weeks','weight_1','tt_2_given','high_risk_during_4th_anc','mother_age_greater_than_25_less_than_30']], logs_train['severe_underweight'])
...
X_train, X_test, y_train, y_test = train_test_split(logs_train[['hb_1','bp_sys_1','registered_within_12_weeks','weight_1','tt_2_given','high_risk_during_4th_anc','mother_age_greater_than_25_less_than_30']], logs_train['severe_underweight'], test_size=0.2, stratify=logs_train['severe_underweight'], random_state=random.seed(1234))


SMOTE_SRF.fit(over_X_train, over_y_train)
#SMOTE SRF prediction result
y_pred = SMOTE_SRF.predict(logs_train[['hb_1','bp_sys_1','registered_within_12_weeks','weight_1','tt_2_given','high_risk_during_4th_anc','mother_age_greater_than_25_less_than_30']])
#Create confusion matrix
fig = plot_confusion_matrix(SMOTE_SRF, logs_train[['hb_1','bp_sys_1','registered_within_12_weeks','weight_1','tt_2_given','high_risk_during_4th_anc','mother_age_greater_than_25_less_than_30']], logs_train['severe_underweight'], display_labels=['Severe underweight', 'Normal'], cmap='Greens')
...
for column in ['severe_underweight','hb_1','bp_sys_1','registered_within_12_weeks','weight_1','distance_from_phc_to_nearest_camp_in_kms','tt_2_given','high_risk_during_4th_anc','mother_age_greater_than_25_less_than_30']:
...
X=part_9[['hb_1','bp_sys_1','registered_within_12_weeks','weight_1','distance_from_phc_to_nearest_camp_in_kms','tt_2_given','high_risk_during_4th_anc','mother_age_greater_than_25_less_than_30']]
...
X_test=rest_part_25[['hb_1','bp_sys_1','registered_within_12_weeks','weight_1','distance_from_phc_to_nearest_camp_in_kms','tt_2_given','high_risk_during_4th_anc','mother_age_greater_than_25_less_than_30']]
...
# title
plt.title('ROC Curve - Severe underweight')
...
rf_10=sensitivity_check(training_features,training_target,0.1,'Severe underweight - Random Forest (10% Over Sampling)',['Severe underweight (Positive)','Normal (Negative)'])
